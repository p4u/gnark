...
func msmChunkedG1(scalars icicle_core.DeviceSlice, bases icicle_core.DeviceSlice, cfg icicle_core.MSMConfig) (curve.G1Jac, int, error) {
	...
	chunks := configureMSM(
		...
		int(unsafe.Sizeof(icicle_bn254.Affine{})),
		int(unsafe.Sizeof(icicle_bn254.Projective{})),
	)
...
func Prove(r1cs *cs.R1CS, pk *ProvingKey, fullWitness witness.Witness, cfg *icicle.Config) (*groth16_bn254.Proof, error) {
	...
	computeBS1 := func() error {
		...
		defer func() {
			if g1BDevice.AsUnsafePointer() != nil {
				g1BDevice.Free()
			}
	...
	computeAR1 := func() error {
		...
		defer func() {
			if g1ADevice.AsUnsafePointer() != nil {
				g1ADevice.Free()
			}
	...
	computeBS2 := func() error {
		...
		defer func() {
			if g2BDevice.AsUnsafePointer() != nil {
				g2BDevice.Free()
			}
	...
	icicle_runtime.RunOnDevice(&device, func(args ...any) {
		...
		// Concurrent AR1 + BS1 execution for improved performance
		// These operations are independent and can run in parallel
		log.Debug().Msg("Starting concurrent AR1+BS1 MSMs")

		chAr1Done := make(chan error, 1)
		chBs1Done := make(chan error, 1)

		go func() {
			icicle_runtime.RunOnDevice(&device, func(args ...any) {
				chAr1Done <- computeAR1()
			})
		}()

		go func() {
			icicle_runtime.RunOnDevice(&device, func(args ...any) {
				chBs1Done <- computeBS1()
			})
		}()

		// Wait for both to complete
		if err := <-chAr1Done; err != nil {
			panic(fmt.Sprintf("concurrent AR1: %v", err))
		}
		if err := <-chBs1Done; err != nil {
			panic(fmt.Sprintf("concurrent BS1: %v", err))
		}

		log.Debug().Msg("Concurrent AR1+BS1 completed successfully")

		// Sequential execution for dependent operations (BS2 and KRS depend on AR1/BS1 results)
